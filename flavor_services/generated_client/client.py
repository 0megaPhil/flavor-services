...
class Client(AsyncBaseClient):
    async def create_player(self, **kwargs: Any) -> CreatePlayer:
        query = gql(
            """
        ...
        response = await self.execute(
            query=query, operation_name="CreatePlayer", variables=variables, **kwargs
        )
        data = self.get_data(response)
        return CreatePlayer.model_validate(data)
